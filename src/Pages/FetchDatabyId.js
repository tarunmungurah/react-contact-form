...
function FetchDatabyId() {
  ...
  useEffect(() => {
    Axios.get(
      'https://bat-recup-staging-backend.cleverapps.io/api/details-fiche/' + id
    )
      .then((res) => {
      ...
      .catch((err) => {
        if (err.code === 'ERR_BAD_REQUEST') setError('ID does not exist!');
        console.log(err);
        return;
      });
  }, [id]);
    ...
    <div className="displayed_data">
      <div className="container">
        <div className="data">
          <div className="id">
            {' '}
            {data.id} <p className="text-danger"> {error}</p>{' '}
          </div>
